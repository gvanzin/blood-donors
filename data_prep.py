...
def load_data():
	...
	#drop columns 
	data.drop(['\xef\xbb\xbfCategory','Country','Attempted_Procedure','Parent_ID','Region_ID','Stamp','Device_ID','Create_Date',\
		'Update_Date','Update_User_Id','ConfigTimeFormat','Procedure_Type'],axis=1, inplace=True)
...
def bin_donation_count(data_balanced):
...
def date_engineering(data_balanced):
...
def date_formatting(data_balanced):
	'''reformat date'''
...
def products(data_balanced):
	...
	data_balanced['Products']=data_balanced['yield_units']+data_balanced['Bag_Volume_units']+\
	data_balanced['Bag_Dose_RBC1_units']
	return data_balanced


def convert_to_sframe(data_balanced):
...
def modeling_prep():
	...
	data_subset=subset_data(data_loaded)
	data_max_timestamp=max_timestamp(data_subset)
	data_with_target=get_target(data_max_timestamp)
	data_binned_donation_count=bin_donation_count(data_with_target)
	data_date_engineered=date_engineering(data_binned_donation_count)
	data_date_formatted=date_formatting(data_date_engineered)
	data_with_products=products(data_date_formatted)
	data_sframe=convert_to_sframe(data_with_products)
	'''remove features not to be used in grid searching'''
	data_sframe.remove_columns(['Raw_Scan_Data', 'Boot_Datetime', 'Account_ID', 'ID', 'Trima_Summary_ID',\
	 'Timestamp', 'count_group'])
	train_set, test_set = data_sframe.random_split(0.8, seed=1)
