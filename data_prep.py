...
def modeling_prep():
	...
	data_max_timestamp=max_timestamp(datasubset)
	data_with_target=get_target(data_max_timestamp)
	data_date_formatted=date_formatting(data_with_target)
	data_binned_donation_count=bin_donation_count(data_date_formatted)
	data_date_engineered=data_date_engineered(data_binned_donation_count)
	data_with_products=products(data_date_engineered)
	data_sframe=convert_to_sframe(data_with_products)
	'''remove features not to be used in grid searching'''
	data_sframe.remove_columns(['Raw_Scan_Data', 'Boot_Datetime', 'Account_ID', 'ID', 'Trima_Summary_ID', 'Timestamp', 'count_group'])
